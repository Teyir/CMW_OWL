...
<?php if($_Joueur_['rang'] == 1 OR $_PGrades_['PermsForum']['general']['modeJoueur'] == true)
 	{
 		?>
 			<p class="text-center">
 				<a href="?action=mode_joueur" class="btn btn-sm btn-theme">Passer en mode visuel <?php echo ($_SESSION['mode']) ? "Administrateur" : "Joueur"; ?></a>
 			</p>
 		<?php 
 	}
...
{ ?>
		...
		<div class="row"><div class="col-md-8"><center><h3><?php echo ucfirst($fofo[$i]['nom']); ?></h3></center></div><?php if(isset($_Joueur_) AND ($_PGrades_['PermsForum']['general']['deleteForum'] == true OR $_Joueur_['rang'] == 1) AND !$_SESSION['mode']){ ?>
		<div class="col-md-4" style="text-align: right;">
			<div class="dropdown">
			  <button class="btn btn-sm btn-theme-inverse" type="button" id="ordreforum<?=$fofo[$i]['id']; ?>" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
			    Modifier l'ordre
			  </button>
			...
			<a href="?action=remove_forum&id=<?php echo $fofo[$i]['id']; ?>" class="btn btn-sm btn-dark" style="text-align: right;">Supprimer</a></div></div><?php } ?></div>
		<thead>
			<tr>
				<th></th>
				<th>Nom</th>
...
<?php   for($j = 0; $j < count($categorie); $j++) { 
				...
				<td><?php 	$sousforum = $bddConnection->prepare('SELECT * FROM cmw_forum_sous_forum WHERE id_categorie = :id_categorie');
						...
						<ul class="dropdown-menu" aria-labelledby="sous-forum<?php echo $categorie[$j]['id']; ?>">
							<?php for($s = 0; $s < count($sousforum); $s++) {
								?><li><a href="?&page=forum_categorie&id=<?php echo $categorie[$j]['id']; ?>&id_sous_forum=<?php echo $sousforum[$s]['id']; ?>"><?php echo $sousforum[$s]['nom']; ?></a></li>
							<?php } ?>
						</ul>
						<?php } ?>
						</div>
			...
			<?php
				...
				?></td>
			<td><p class="text-right"><a href="?&page=forum_categorie&id=<?php echo $categorie[$j]['id']; ?>"><?php echo $_Forum_->compteTopicsForum($categorie[$j]['id']); ?></a></p></td><?php
...
{
	?><a class="btn btn-primary btn-xs btn-block" style="background-color: #2e71f0;" role="button" data-toggle="collapse" href="#add_forum" aria-expanded="false" aria-controls="add_forum">
...
<div class="collapse" id="add_forum">
	<div class="well">
		<form action="?action=create_forum" method="post">
			...
			<br/>
			<p class="text-right">
				<button type="submit" class="btn btn-md btn-theme">Créer une catégorie</button>
			</p>
		</form>
	</div>
</div><br><?php
...
{
	?>
	<a class="btn btn-primary btn-xs btn-block" role="button" style="background-color: #2e71f0;" data-toggle="collapse" href="#add_categorie" aria-exepanded="false" aria-controls="add_categorie"> Ajouter un Forum</a>
	<div class="collapse" id="add_categorie">
			<form action="?action=create_cat" method="post"><br>
				...
				<p class="text-right">
					<button type="submit" class="btn btn-md btn-theme">Créer un Forum</button>
				</p>
			</form>
	</div>
<?php
