...
if(isset($_GET['id']) AND isset($_Joueur_))
{
		...
		<section class="page-tittle page-tittle-lg bg dark-overlay" style="background-image: url(<?php echo $image_entete_page; ?>)">
			<div class="container">
			    ...
			    <ol class="breadcrumb pull-right mrg-top-30">
				  ...
				  <li class="breadcrumb-item active"><?php echo $topicd['nom']; ?></li>
				</ol>
			</div>
		</section>
        <!-- Page Tittle End -->
	...
	<nav class="nav nav-pills nav-justified">
		...
		{
			?>
			<a class="btn btn-sm btn-dark" href="?&action=unfollow&id_topic=<?php echo $topicd['id']; ?>">Ne plus suivre cette discussion </a>
				<?php
		}
		else
		{
			?>
			<a class="btn btn-sm btn-dark" href="?&action=follow&id_topic=<?php echo $topicd['id']; ?>">Suivre cette discussion</a>
				<?php
		}
		?>
		<a class="btn btn-sm btn-dark" href="?&page=forum_categorie&id=<?php echo $topicd['id_categorie']; if(isset($topicd['sous_forum'])) { ?>&id_sous_forum=<?php echo $topicd['sous_forum']; } ?>">Revenir à l'index de la catégorie</a>
		<a class="btn btn-sm btn-dark" href="?&page=forum">Revenir à l'index du forum</a>
	</nav><br/>
	...
	<div class="row">
		<div class="col-lg-8">
			<div class="dropdown">
				<button class="btn btn-sm btn-dark" type="button" style="color: #fff;background-color: #c9302c;border-color: #761c19;" id="Actions-Modération" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">
	...
	<form action="?&action=signalement_topic" method="post">
		...
		<button type="submit" class="btn btn-sm btn-theme">Signaler !</button>
	</form>
	</div>
	<?php if($_Joueur_['pseudo'] == $topicd['pseudo'] OR ($_Joueur_['rang'] == 1 OR $_PGrades_['PermsForum']['general']['editTopic'] == true) AND !$_SESSION['mode'])
	{
		?><div class="col-md-2"><form action="?page=edit_topic" method="post">
			...
			<button type="submit" class="btn btn-md btn-dark">Editer le topic</button>
		</form>
		</div>
		<?php 
	}
		...
		<form action="?action=remove_topic" method="post">
			<input type="hidden" name="id_topic" value="<?php echo $id; ?>" />
			<a class="btn btn-md btn-dark" role="button" data-toggle="collapse" href="#topic_<?php echo $id; ?>" aria-expanded="false" aria-controls="collapseExample" >
				Supprimer ce topic ? </a>
				<div style="width: 155%;" class="collapse" id="topic_<?php echo $id; ?>">
					<div class="well">
						<button type="submit" class="btn btn-sm btn-theme">Confirmer la suppression du topic </button>
					</div>
				</div>
		</form>
		</div><?php
		...
		<div class="col-md-2">
			...
			if($_Joueur_['pseudo'] === $answerd[$i]['pseudo'] OR ($_PGrades_['PermsForum']['moderation']['editMessage'] == true OR $_Joueur_['rang'] == 1) AND !$_SESSION['mode'])
			{
				?><div class="col-md-2"><form action="?page=edit_answer" method="post">
					<input type="hidden" name="id_answer" value="<?php echo $answerd[$i]['id']; ?>" />
					<button type="submit" class="btn btn-md btn-dark">Editer ce message</button>
				</form></div>
				<?php 
			}
			...
			<div class="col-md-2">
				<form action="?action=remove_answer" method="post">
					...
					<a role="button" class="btn btn-md btn-theme" data-toggle="collapse" href="#answer_<?php echo $answerd[$i]['id']; ?>" aria-expanded="false" aria-controls="collapseExample">
	...
	<form action="?&action=post_answer" method="post">
		...
		<div class="form-group row">
			<div class="col-md-12 text-center">
				<button type="submit" class="btn btn-md btn-theme">Poster votre réponse</button>
			</div>
		</div>
		</p>
	</form>
